...
| --- | --- | --- | --- | --- | --- | --- | --- |
| IDM-01 | SAP -> IDM Production & Factory Dispatch Import | Agreed | Partially Implemented / Integrated for production import | 2026-06-08 | TEC-DB, TEC-AUTH, TEC-INT; OI-7 for full SAP contract | Backend route/service tests; frontend Import Monitor tests; API module tests | `POST /api/idm-01/import/production` imports production serials with idempotent batches, serial creation, serial events, duplicate-row handling, and unknown-product rejections. Import hardened with HMAC-SHA256 webhook verification, source labelling, rejection detail, and Import Monitor UI. SAP adapter build blocked on OI-7. Factory-dispatch/invoice inbound transport is not implemented beyond schema/seed foundations. |
...
| TEC-INT | SAP / external integration layer | Partially Implemented | 2026-06-08 | IDM-01 import service tests and integration batch repository tests | Inbound webhook endpoint hardened. Ageing SAP export endpoint ready. Confirmed-serial outbound stub endpoints ready. Import Monitor UI operational. Adapter transport blocked on OI-7. |
...
| 2026-06-08 | SAP integration readiness pass: HMAC webhook verifier, import monitor UI, ageing SAP export endpoint, confirmed-serial outbound stub, integration batch history with rejection detail. All adapter-ready; transport build pending OI-7. |
