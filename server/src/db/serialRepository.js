export function createSerialRepository(pool) {
  ...
  return {
    ...
    async insertProductionSerial({
      ...
      // Conflict: the row already exists. serial_no is stored composed (V027)
      // while `serialNo` here is the raw base, so resolve by base scoped to the
      // product (never ambiguous because product_id is known).
      const existing = await this.findBySerialNo(serialNo, { productId });
      return existing && existing.ambiguous ? null : existing;
    },
    ...
    // Same availability rule as countAvailableStock, broken down per product so
    // the dispatch/transfer UI can show how much of each invoice item is
    // actually in the warehouse. Returns Map<productId, count>.
    async countAvailableStockByProduct({ warehouseId, productIds }) {
      if (!Array.isArray(productIds) || productIds.length === 0) {
        return new Map();
      }

      const result = await pool.query(
        `SELECT product_id AS "productId", COUNT(*)::int AS count
         FROM serial_master
         WHERE current_status = 'IN_STOCK'
           AND current_warehouse_id = $1
           AND product_id = ANY($2::bigint[])
      ...
      return new Map(result.rows.map((row) => [Number(row.productId), row.count]));
    },
    ...
    // Resolve a scanned serial to its serial_master row. Since V027 the stored
    // serial_no is composed (`<PREFIX>_<base>`) while operators scan the raw base
    // serial, so we try an exact serial_no match first (legacy rows / a scan of
    // the full composed code) then fall back to base_serial. A base serial can be
    // shared by multiple products; when it maps to more than one row and no
    // productId was supplied to disambiguate, return { ambiguous, matches }.
    async findBySerialNo(serialNo, { productId } = {}) {
      const columns = `
           serial_id AS "serialId",
           serial_no AS "serialNo",
           base_serial AS "baseSerial",
      ...
      const exact = await pool.query(
        `SELECT ${columns} FROM serial_master WHERE serial_no = $1`,
        [serialNo]
      );
      if (exact.rows[0]) {
        return mapSerial(exact.rows[0]);
      }

      const params = [serialNo];
      let where = "base_serial = $1";
      ...
      const byBase = await pool.query(
        `SELECT ${columns} FROM serial_master WHERE ${where}`,
        params
      );
      ...
      if (byBase.rows.length > 1) {
        return {
          ambiguous: true,
          matches: byBase.rows.map((row) => ({
            serialId: toNumber(row.serialId),
      ...
      return mapSerial(byBase.rows[0]);
    }
  };
}
